...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM tutoriales WHERE id = ?");
    $stmt->execute([$id]);
    $tutorial = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titulo) || empty($descripcion) || empty($contenido_html)) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE tutoriales 
                SET categoria_id = ?, titulo = ?, descripcion = ?, contenido_html = ?, estado = ?, v24_5_compatible = ? 
                WHERE id = ?
            ");
            $stmt->execute([
                $categoria_id, 
            ...
            $error = 'Error al actualizar el tutorial: ' . $e->getMessage();
        }
    }
}

$page_title = "Editar Tutorial";
$header_title = "Editar Contenido";
...
<div class="max-w-4xl mx-auto">
    <nav class="flex items-center gap-xs text-on-surface-variant font-label-md text-label-md mb-lg">
        ...
        <span class="text-on-surface">Editar Tutorial</span>
    </nav>

    <div class="bg-surface-container rounded-xl p-xl border border-outline-variant shadow-lg">
        <h2 class="font-headline-lg text-headline-lg text-on-surface mb-xl">Editar Tutorial</h2>
        ...
        <form method="POST" class="space-y-lg">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
                <div>
                    ...
                    <input type="text" id="titulo" name="titulo" required value="<?php echo htmlspecialchars($tutorial['titulo']); ?>"
                ...
                <div>
                    ...
                    <select id="categoria_id" name="categoria_id" required
                        ...
                        <?php foreach ($categorias as $cat): ?>
                            <option value="<?php echo $cat['id']; ?>" <?php echo ($cat['id'] == $tutorial['categoria_id']) ? 'selected' : ''; ?>>
                                <?php echo $cat['nombre']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="2" required
                    class="w-full bg-surface-container-highest text-on-surface border border-outline-variant rounded-lg px-md py-3 focus:outline-none focus:border-primary-container transition-all"><?php echo htmlspecialchars($tutorial['descripcion']); ?></textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="contenido_html" name="contenido_html" rows="15" required
                    class="w-full bg-surface-container-highest text-on-surface border border-outline-variant rounded-lg px-md py-3 focus:outline-none focus:border-primary-container transition-all font-mono text-sm"><?php echo htmlspecialchars($tutorial['contenido_html']); ?></textarea>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-xl bg-surface-container-low p-md rounded-lg border border-outline-variant">
                <div>
                    ...
                    <select id="estado" name="estado" class="bg-surface-container-highest text-on-surface border border-outline-variant rounded-lg px-md py-2 focus:outline-none">
                        <option value="publicado" <?php echo ($tutorial['estado'] === 'publicado') ? 'selected' : ''; ?>>Publicado</option>
                        <option value="pendiente" <?php echo ($tutorial['estado'] === 'pendiente') ? 'selected' : ''; ?>>Borrador / Pendiente</option>
                    </select>
                </div>
                <div class="flex items-center gap-md pt-6">
                    <input type="checkbox" id="v24_5" name="v24_5" <?php echo ($tutorial['v24_5_compatible']) ? 'checked' : ''; ?> class="w-5 h-5 rounded border-outline-variant text-primary focus:ring-primary bg-surface-container-highest">
